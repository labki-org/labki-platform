...
### Added
- **Labki Forum — forum subscriptions via auto-watch**: a new `PageSaveComplete` hook in `extensions.platform.php` copies the forum landing page's watchers onto each newly-created topic. Clicking the Watch star on `Forum:Miniscopes` (or `2026_Paris_Workshop:Forum`) now produces a real "subscribe to this forum" experience — bell notification + email (with `$wgEnotifWatchlist = true`) when anyone posts a new topic, plus on subsequent replies via the standard watchlist path. Leaf-only by design (mirrors DT's per-page subscription model); existing topics are not retroactively added to existing watchers. See `docs/labki-forum.md` "Subscribing to a forum" section.
