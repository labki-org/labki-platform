...
### Changed
- **Labki Forum — `Comment count` / `Participant count` / `Topic starter` now driven by DiscussionTools' `CommentParser`** instead of a wikitext regex. The original `ParserAfterParse` annotator counted `(UTC)` signature timestamps via regex, which silently undercounted on any wiki with a non-UTC `$wgLocaltimezone` (signatures end in `(PDT)` / `(PST)` / `(EST)` / etc.) — reproduced on miniscope.org, where `Forum:Home` reported `0 Replies` on topics with real ones. The new flow defers the DT parse: `PageSaveComplete` schedules a `labkiForumDTAnnotateJob`, the job runs DT's `parseRevisionParsoidHtml`, caches `{count, people, starter}` in `MainObjectStash`, and runs `RefreshLinksJob` inline. `ParserAfterParse` then reads the stash and contributes the three DT-derived properties through SMW's normal parser-data channel, so any subsequent re-parse (purge, cache eviction) re-emits the same locale-correct values. Counts now match exactly what DT itself shows in its bell / subscription UI, and `Has forum` / `Topic subject` remain title-derived as before. Backfill existing topics by running `php maintenance/runJobs.php` (or just edit each topic once). See [`docs/labki-forum.md`](docs/labki-forum.md) "DT-derived counts land asynchronously" for the full timing model and failure modes.
